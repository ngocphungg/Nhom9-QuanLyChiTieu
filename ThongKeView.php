...
<body>
        ...
        <ul>
           <li><a class="dropdown"href="TrangChuLogin.php">Trang chủ</a></li>
			...
			<div class="dropdown">
			  ...
			  <div class="dropdown-content">
				<a class="active" href="ThongKeView.php">TThống kê Thu/Chi</a>
	...
	<article>
	  <h1>THỐNG KÊ THEO THỂ LOẠI </h1>
		<h3><a href="ThongKeChi.php">Thống kê Chi tiêu</a><a href="ThongKeView.php" >Thống kê Thu Nhập</a></h3>
    ...
    // Tính toán thống kê theo danh mục
	 $sqlthu = "SELECT danhmuc.id_type, danhmuc.name, SUM( amount) AS sumthu FROM giaodich JOIN danhmuc ON giaodich.id_type = danhmuc.id_type WHERE giaodich.kieu='Thu' GROUP BY danhmuc.id_type, danhmuc.name";
    $result = $conn->query($sqlthu);
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            array_push($totals, $row["sumthu"]);
            echo "<tr><td>".$row["name"]."</td><td>".$row["sumthu"]."</td></tr>";
        }
        echo "</table>";
        ...
        echo "        datasets: [{";
        echo "            label: 'Category Totals',";
        echo "            data: " . json_encode($totals) . ",";
